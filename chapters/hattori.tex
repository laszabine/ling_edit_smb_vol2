...
\ex \label{2ha}
\begin{xlist}
\ex[]{\label{2aha}It is tough to please linguists.}
\ex[]{\label{2bha}To please linguists is tough.}
\end{xlist}
\ex \label{3ha}
\begin{xlist}
\ex[*]{\label{3aha}It is pretty to look at these flowers.}
\ex[*]{\label{3bha}To look at these flowers is pretty.}
\end{xlist}
\end{exe}
...
\begin{xlist}
\ex \label{12aha}
\gll [kono te-no hanzai]$_{i}$ –ga (keisatu-nitotte) [\textsubscript{NP} [$_{S’}$ e$_{j}$ e$_{i}$ okasi-ta] ningen$_{j}$]-o  sagasi-yasu-i\\
...
\ex \label{12bha}
\gll [kooitta     itazura]$_{i}$ -ga (senseigata-nitotte) [\textsubscript{NP} [$_{S'}$ e$_{j}$ e$_{i}$ sita] seito$_{j}$]-o    mituke-yasu-i\\
...
\ex \label{12cha}
\gll [Sooiu ronbun]$_{i}$ -ga (watasi-nitotte) [\textsubscript{NP}[$_{S'}$ e$_{j}$ e$_{i}$ kai-ta] gakusei$_{j}$]-o  hyookasi-niku-i\\
...
\begin{xlist}
\ex[*]{\label{13aha}
\gll [\textsubscript{PP} Anna taipu -no zyosei-to]$_{i}$ -ga (John-nitotte) [\textsubscript{NP}[$_{S'}$ e$_{j}$ e$_{i}$ kekkon-site-i-ru] otoko$_{j}$]-to hanasi-niku-i.\\
{} that  type of woman-with -\textsc{nom} John-for {} {} {} marry-\textsc{prs} man-with talk-hard-\textsc{prs}\\
\glt (lit.) ‘[With that type of woman]$_{i}$ is hard (for John) to talk to [\textsubscript{NP} the man [$_{S’}$ who marry e$_{i}$ ]].’\\
cf. \gll [\textsubscript{PP} Anna taipu -no zyosei-to]$_{j}$ -ga (John$_{i}$-nitotte) [$_{S'}$ pro$_{i}$ e$_{j}$] kekkonsite-mo-i-i  to] tomodachi-ni ii-niku-i.\\
{} that  type of woman-with -\textsc{nom} John-for {} {} {} marry-may-\textsc{prs} \textsc{comp} friend-to say-hard-\textsc{prs}\\
\glt (lit.) ‘[With that type of woman]$_{i}$ is hard (for John$_{j}$) to say to his friends [$_{S’}$ that he$_{j}$ may marry e$_{i}$]’}
\ex[?*]{\label{13bha}
\gll [\textsubscript{PP} Sooiu kin'yuukikan-kara]$_{i}$  -ga     (John-nitotte) [\textsubscript{NP} [$_{S'}$ e$_{j}$ itumo e$_{i}$ okane-o      takusan karite-i-ru]    hito$_{j}$]-o  sin'yoosi-niku-i.\\
{} such financial.agency-from -\textsc{nom} John-for {} {} {} always {} money-\textsc{acc} a.lot borrow-\textsc{prs} person-\textsc{acc} trust-hard-\textsc{prs}\\
\glt (lit.) ‘[From such a financial agency]$_{i}$ is hard (for John) to trust [\textsubscript{NP} a person [$_{S’}$ who always loans a lot of money t$_{i}$]].’\\
cf. \gll [\textsubscript{PP} Sooiu kin'yuukikan-kara]$_{j}$ -ga (John$_{j}$-nitotte) [$_{S'}$ pro$_{i}$ e$_{j}$ okane-o takusan karite-i-ru to] ii-niku-i\\
{} such financial.agency-from -\textsc{nom} John-for {} {} {} money-\textsc{acc} a.lot borrow-\textsc{prs} \textsc{comp} say-hard-\textsc{prs}\\
\glt (lit.) ‘[From such a financial agency]$_{i}$ is hard (for John) to say [$_{S'}$ that he has loaned a lot of money e$_{i}$]’ }
\end{xlist}
\end{exe}
...
In order to check the subjacency effect, I will use the translation of \citeauthor{Chomsky1977}’s (\citeyear{Chomsky1977}) examples regarding the locality in English \textit{tough} constructions, i.e. (\ref{17cha}).\footnote{In \REF{17ciiha}, \textit{which} sonatas is moving past a null \textit{wh} operator (i.e. CNO in our analysis), resulting in a \textit{wh}-island constraint violation.}

\begin{exe}
\ex \label{17ha}
\begin{xlist}
\ex[]{\label{17aha}John$_{i}$ is easy (for us) to please t$_{i}$}
\ex \label{17bha}
\begin{xlist}
\ex[]{\label{17biha}John$_{i}$ is easy (for us) [to convince Bill [to do business with t$_{i}$]]}
\ex[]{\label{17biiha}John$_{i}$ is easy (for us) [to convince Bill [that he should meet t$_{i}$]]}

\end{xlist}
\ex \label{17cha}
\begin{xlist}
\ex[*]{\label{17ciha}John$_{i}$ is easy (for us) [to describe to Bill [a plan [to assassinate t$_{i}$]]]\jambox*{(Complex NP)}}

\ex[*]{\label{17ciiha}Which sonatas$_{i}$ are the violin$_{j}$ easy [to play t$_{i}$ on t$_{j}$] \jambox*{(Wh-island)}}
\end{xlist}

\ex 
\begin{xlist}
\ex[]{The violin$_{j}$ is easy [\textsubscript{CP} [\textsubscript{CNO} Op t$_{j}$ ]$_{k}$ \sout{for} \textsc{pro} to play sonatas on t$_{k}$].}

\ex[*]{Which sonatas$_{i}$ are the violin$_{j}$ easy [\textsubscript{CP} [\textsubscript{CNO} Op t$_{j}$ ]$_{k}$ \sout{for} \textsc{pro} to play t$_{i}$ on t$_{k}$].}
\end{xlist}
\end{xlist}
\end{exe}
...
\begin{xlist}
\ex[]{\label{21aha}
\gll Er kam und (er) besuchte die Kinder.\\ 
he.\textsc{nom} came and (he) visited the children\\}

\ex[]{\label{21bha}
\gll Er kam und (er) wurde verhaftet.\\
he came and (he) was arrested\\\jambox*{\citep[477]{ZaenenThrainsson1985}}}
...
\glt `He survived and \underline{\hspace{6mm}} was easy to help.'}       
...
\glt `This conflict worsened and is difficult to solve.'}
...
\glt ‘It is easy to describe a plan to kill John’}
...
\glt ‘*John is easy (for us) to describe a plan to kill’}
...
\glt ‘[from this bank] is hard to trust a person who loans a lot of money \textit{t$_{i}$}’}
...
\begin{xlist}
\ex[]{\label{35aha}
\gll Lako nam je Borisu prepričati trač    da   su  ubili \textit{njega}. \\
easy us.\textsc{dat} is Boris.\textsc{dat} retell gossip that are kill him.\textsc{acc}\\
\glt `It is easy for us to retell to Boris a gossip that they killed him.'}

\ex[*]{\label{35bha}\textit{Njega}$_{i}$ je nama lako Borisu prepričati trač da su ubili t$_{i}$.}
\end{xlist}
\end{exe}
...
Based on the diagnostics (\ref{16ha}), the \textit{tough} constructions in the 13 languages surveyed are categorized into 3 types (\tabref{tab:1:types}).
...
As shown in \tabref{tab:1:types}, the type (i) “\textit{tough}” constructions (where the CNO movement is involved) are available in a limited number of languages including English. Recall now that our prediction was that English-like \textit{tough} constructions are available only in DP-languages based on the CNO analysis of \textit{tough} constructions where the presence of the DP layer is crucial for the CNO to smuggle the \textit{tough} subject. In this regard, the NP/DP distinction and the availability of the type (i) \textit{tough} constructions in the languages under consideration are summarized in \tabref{tab:2:npdp}. 
...
\ex \label{36ha}
\begin{xlist}
\ex[*]{\label{36aha}Betsy$_{i}$ is easy [Op$_{i}$ [ \textsc{pro} to expect [ t$_{i}$ fixed the car] ] ].}
\ex[*]{\label{36bha}John is easy [Op$_{i}$ [\textsc{pro} to believe [ t$_{i}$ kissed Mary] ] ].}
\ex[??]{\label{36cha}This car is hard [Op$_{i}$ [ \textsc{pro} to claim [ [ Betsy fixed t$_{i}$ ]] ]].}
\ex[??]{\label{36dha}That language is impossible [Op$_{i}$ [\textsc{pro} to say [ [ Greg will learn t$_{i}$ ]] ]].  \hfill \citep[477]{Stowell1986}}                                        
\end{xlist}
\end{exe}
