...
\ex. Relativized Minimality (Rizzi \citeyear{Rizzi:1990}: taken from Rizzi \citeyear{Rizzi:2001a})
\a. ..... X .... Z .... Y\\
\b. Y is in a Minimal Configuration (MC) with X iff there is no Z such that\\
(i) Z is of the same structural type as X, and\\
(ii) Z intervenes between X and Y\\
\c. The typology must involve at least two irreducible\label{ex:RMc}
distinctions:\\
...
\qtreecenterfalse
\ex. \label{ex:extproj}\footnotesize{ \Tree [.\colorbox{Gray}{XP} \framebox{Spec}  [.\colorbox{Gray}{X$'$} \colorbox{Gray}{X} [.\colorbox{Gray}{YP} \framebox{Spec}  [.\colorbox{Gray}{Y$'$} \colorbox{Gray}{Y} \framebox{\colorbox{Gray}{ZP}}  ]]]]}
...
\ex. VP fronting \label{ex:VPfront}
\a.  ... and $[$$_{CP}$ $[$$_{VP}$ do their homework ] $[$$_{TP}$ they will \underline{\hspace{.5cm}} $]]$ \hfill \textsc{English}\label{ex:EngVP}\\
\bg.  $[$$_{CP}$ $[$$_{VP}$ Das Buch gelesen $]$  hat  $[$$_{TP}$ Peter  gestern \underline{\hspace{.5cm}} $]]$ \label{ex:VPGerm}\\
~ ~ the book read ~ has ~ Peter yesterday\\
`Peter  read  the  book  yesterday' \hfill \textsc{German}\\
\cg. $[$$_{CP}$ $[$$_{VP}$ nggotong watu-ne $]$ $[$$_{TP}$ cak Kholiq iso \underline{\hspace{.5cm}} $]]$
\label{ex:VPJavanese}\\
~ ~ \textsc{av}.lift rock-\textsc{def} ~  ~ Mr. Kholiq \textsc{circ.pos}\\
`Lift the stone, Kholiq can.'  \hfill \textsc{Paciran Javanese}\\
...
\ex. Niuean Pseudo Noun Incorporation (Massam \citeyear{Massam:2001}:157) \label{ex:Niuean}
 \ag. [ Takafaga t$_k$ ] t\=umau n\=i e \textbf{ia} [ \uuline{e} \uuline{tau}  \uuline{ika} ]$_k$\label{ex:NnoPNI}\\
~ hunt ~ ~ aways \textsc{emph} \textsc{erg} he ~ \textsc{abs} \textsc{pl} fish\\
`He is always fishing.' \hfill [ V t$_k$ ]$_j$ \textbf{S} \uuline{O}$_k$ t$_j$ \\
\vspace{.25cm}
\bg. [ Takafaga  \uuline{ika} ] t\=umau n\=i a \textbf{ia}\label{ex:NPNI}\\
~ hunt fish ~ always \textsc{emph} \textsc{abs} he  \\
`He is always fishing.' \hfill [ V  \uuline{O} ]$_j$  \textbf{S}  t$_j$
...
\ex. Icelandic (Holmberg \citeyear{Holmberg:1986}: 166)
\ag. Hvers vegna lasu st\'udentarnir ekki allir \textbf{greinina}\label{ex:icelandica}\\
why ~ read the.students not all the.article\\
`Why didn't all the students read the article?'\footnote{A translation is not given in the original article.  This translation is taken from \url{https://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.491.8626&rep=rep1&type=pdf}.}
\b. Hvers vegna lasu st\'udentarnir \textbf{greinina} ekki allir \label{ex:icelandicb}
...
\needspace{5\baselineskip}
\ex. Malagasy (Inverse) rightward object shift
\ag.  Nijinja \textit{(ny)} \textit{vary} \textbf{haingana} ny mpamboly\label{ex:bareNPgasy}\\
\textsc{pst-at.}cut (\textsc{det}) rice quickly \textsc{det} farmer\\
`The farmer harvested (the) rice quickly.' \\
\bg.  Nijinja   \textbf{haingana} *(\textit{ny}) \textit{vary} ny mpamboly\label{ex:MalOS}\\
\textsc{pst-at.}cut  quickly ~\textsc{det} rice \textsc{det} farmer\\
`The farmer harvested *(the) rice quickly.' \\
\c. \ [[ V t$_k$ ]$_j$ \textbf{Adv} \uuline{O}$_k$ t$_j$ ]$_m$ \textbf{S} t$_m$
...
\needspace{5\baselineskip}
\ex. Niuean vs.\ Malagasy VP fronting\label{ex:NvsM}\footnote{Labels of X(P) and Y(P) are used to abstract away from details that are not necessary to make the point needed here.} \label{ex:NvsMtrees}\\\\
\vspace{.25cm}
\scriptsize{
...
\ex. A-movement (obligatory and local) -- inherent feature\\ 
...
\ex. \=A-movement (optional and less local) -- optional (movable) feature\\
\vspace{.2cm}
...
\qtreecenterfalse
\ex. Grimshaw (\citeyear{Grimshaw:2000}:118) \label{ex:Grimshaw}\\\\
\scriptsize{
...
\exg.\textbf{l\=i} [$_{TP}$ \textvbaraccent{O} d\=a s\textvbaraccent{a}k\'a \textbf{l\=i} ]\label{ex:VataPC}\\
eat ~ s/he \textsc{perf.aux} rice eat\\
`S/he has \textbf{eaten} rice.' \hfill Vata: adapted from Koopman (\citeyear{Koopman:1984}:38)\\

\ex. \label{ex:VataVmove}
\ag. [$_{TP}$ \'a l\=a s\textvbaraccent{a}k\'a \textbf{l\=i} ]\\
~ we \textsc{perf.aux} rice eat\\
`We have eaten rice'
\bg. [$_{TP}$ \'a \textbf{l\`i}$_k$ s\textvbaraccent{a}k\'a t$_k$ ] \label{ex:VataVmove}\\
~ we ate rice\\
`We ate rice'  \hfill Vata: adapted from Koopman (\citeyear{Koopman:1984}:42)
...
\exg.  Essen hot Max  gegessen a fish\label{ex:Yiddish}\\
to.eat has Max eaten a fish\\
`As for eating, Max has eaten a fish.'\hfill Yiddish (Cable \citeyear{Cable:2004}:2)
...
\ex.\label{ex:LHMtoC}
\ag. L-ar \textbf{bate} Dumnezeu.\\
him-would-3s punish God\\
`God would punish him.'\\
\bg. \textbf{Bate}-l-ar Dumnezeu.\\
punish-him-would-3s God\\
`God would punish him!' \hfill Romanian: Rivero \citeyear{Rivero:1994}:86
...
\ex.
\ag.  \textbf{Pro\v{c}el} s\v{u}m knigata.\\
read I-have book-the\\
`I have read the book.'\\
\b. *S\v{u}m \textbf{pro\v{c}el} knigata. \hfill Bulgarian: Rivero \citeyear{Rivero:1994}:89
...
\ex. \Tree [.TP \qroof{Buku itu}.DP  [.T$'$ [.T [.D ku$_k$ ] [.V pukul$_i$ ] ] [.\textit{v}P [.DP D\\t$_k$ ] [.\textit{v}$'$ \textit{v}\\t$_i$ \qroof{... Vt$_i$ ...}.VP ]]]]
...
\ex. N-bonding in Malagasy
\ag. \textit{Mamaky} ny boky \textit{aho}.\\
\textsc{pres.av}.read \textsc{det} book \textsc{1sg}\\
`\underline{I} read the book.'\hfill \textit{m-an-vaky}\\
\bg. \textit{Vaki\textbf{ko}} ny boky.\label{ex:Npron}\\
\textsc{pres.ov}.read-\textsc{1sg} \textsc{det} book\\
`I read \underline{the book}.'\hfill \textit{vaki-na} + \textit{ko}\\
\cg. \textit{Vakin-\textbf{dRasoa}} ny boky.\\
`\textsc{pres.ov}.read-Rasoa \textsc{det} book\\
`Rasoa reads \underline{the book}.'\hfill \textit{vaki-na} + \textit{Rasoa}\\
\dg.  \textit{Vakin'\textbf{ny}} vehivavy ny boky.\\
\textsc{pres.ov}.read'\textsc{det} woman \textsc{det} book\\
`The woman reads \underline{the book}.'\hfill \textit{vaki-na} + \textit{ny}
...
\exg. Vaki\textit{ko} *(\textit{izaho}) ankizy ny boky.\label{ex:NAPC}\\
\textsc{pres.ov}.read-\textsc{1sg}  \textsc{1sg.nom} child \textsc{det} book\\
`I child read \underline{the book}.'\hfill Malagasy APC
...
\ex.
\ag. [ \textbf{Mit} \textbf{wem} ] glaubt Hans [$_{CP}$ \textbf{da\ss{}} [$_{IP}$ Jakob jetzt \textbf{t$_i$} spricht ]]\label{ex:fullXP}\\
~ with whom ~ thinks Hans ~ that ~ Jakob now ~ talks\\
`With whom does Hans think that Jakob is now talking?'\\
\bg. \textbf{Was}$_i$  glaubt Hans [$_{CP}$ [ \textbf{mit} \textbf{wem} ]$_i$ [$_{IP}$ Jakob jetzt \textbf{t$_i$} spricht ]]\label{ex:partialX}\\
...
\exg. dare-ga t$_i$ hon-o kaimasita ka$_i$\\
who-\textsc{nom} ~ book-\textsc{acc} bought.\textsc{polite} Q\\
`Who bought the book?'\hfill Japanese: Hagstrom (\citeyear{Hagstrom:2000}:ex.(1))
...
\needspace{5\baselineskip}
\ex. Disjunctive \textbf{ka} \label{ex:disjunctiveka}
\ag. ?*[ John-\textbf{ka} Bill ]-ga \textit{nani-o}  \textbf{t$_i$} nomimasita \textbf{ka}?\\
~ John-or Bill- \textsc{nom} what-\textsc{acc} ~ drank Q?\\
\bg. \textit{nani-o}  \textbf{t$_i$} [ John-\textbf{ka} Bill ]-ga nomimasita \textbf{ka}? \label{ex:Jdisj}\\
 what-\textsc{acc} ~  ~ John-or Bill- \textsc{nom} drank Q?\\
 `What did John or Bill drink?' \hfill Japanese: Hagstrom (\citeyear{Hagstrom:2000}:ex.(2))\\

\ex. Indefinite \textbf{ka} \label{ex:indefiniteka}
\ag.??dare\textbf{ka}-ga \textit{nani-o} \textbf{t$_i$} nomimasita \textbf{ka}?\\
someone what-\textsc{acc} ~  drank Q?\\
\bg.  \textit{nani-o} \textbf{t$_i$} dare\textbf{ka}-ga nomimasita \textbf{ka}? \label{ex:Jindef}\\
 what-\textsc{acc} ~ someone  drank Q?\\
`What did someone drink?' \hfill Japanese: Hagstrom (\citeyear{Hagstrom:2000}:ex.(3))
...
%\needspace{5\baselineskip}
\ex. 
\ag. *Manger\`o [quanto pane] vorrai [t]\\
I-will-eat how-much bread you-will-want\\
\bg. Mi chiedo [quanto pane] vorrai [t]\\
I wonder how-much bread you-will-want\\
\cg. Manger\`o [quanto] vorrai [t]\\
I-will-eat what you-will-want\\

\ex. 
\a. *I shall visit [what town] you will visit [t]
\b. I wonder [what town] you will visit [t]
\c. I shall visit [what] you will visit [t]
...
\ex. \textbf{Extension Condition} (Chomsky \citeyear{Chomsky:1995})\\
